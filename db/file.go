 ...
 * 更新文件信息到数据库中
 */
func UpdateSQL(filehash string, filename string, filesize int64, fileaddr string) bool {
	stmt, err := mydb.DBConn().Prepare("UPDATE file SET file_name=?,file_size=?,file_addr=? WHERE file_sha1=?")
	if err != nil {
		fmt.Printf("Fail to prepare statement,error:", err.Error())
		return false
	}
	...
	result, err := stmt.Exec(filename, filesize, fileaddr, filehash)
	if err != nil {
		fmt.Printf("Fail to execute sql, error:", err.Error())
		return false
	}
	if affected, err := result.RowsAffected(); err == nil {
		if affected > 0 {
			fmt.Printf("File with hash:%s has benn update successfully. ", filehash)
		}
		return true
	}
	return false
